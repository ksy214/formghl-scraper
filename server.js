...
async function extractViaCSS(url) {
  ...
  for (const match of html.matchAll(/<link[^>]+rel=["']stylesheet["'][^>]+href=["']([^"']+)["']/gi)) {
    try {
      cssUrls.add(new URL(match[1], baseUrl).href)
    } catch {}
  }
  ...
  const cssResults = await Promise.allSettled(
    [...cssUrls].slice(0, 5).map(async (cssUrl) => {
      ...
      if (!response.ok) {
        throw new Error(`CSS request failed with ${response.status}`)
      }
  ...
  if (!brandColor) {
    ...
    brandColor =
      Object.entries(colorCount).sort((a, b) => b[1] - a[1])[0]?.[0] || null
  }
  ...
  const fontFamily = fontMatch
    ? resolveCssValue(fontMatch[1].trim(), cssVars)
    : null

  const radiusMatch = cssText.match(
    /border-radius:\s*(\d+(?:\.\d+)?(?:px|rem|em))/i
  )
...
function extractComputedTokens() {
  ...
  return {
    ...
    sectionBg: sectionElement
      ? effectiveBackground(sectionElement)
      : null,
...
function isUsableCssTheme(tokens) {
  ...
  const hasVerifiedButton = Boolean(
    tokens.btnBg &&
      ...
      !isTransparentServerSide(tokens.btnBg) &&
      tokens.detected?.buttonTag &&
      tokens.detected?.buttonText &&
      (tokens.confidence?.button ?? 0) >= 0.7
  )
